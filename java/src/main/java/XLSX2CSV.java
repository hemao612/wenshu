...
//            }
//            output.append('\n');
            output.append("index:").append(String.valueOf(rowNum)).append("\n");
            if (rowNum > 0) {
                String[] parseLine = parser.parseLine(line);
                Row row = sheet.createRow(sheetRow);
                for (int i = 0; i < parseLine.length; i++) {
                    row.createCell(i).setCellValue(parseLine[i]);
                }
                sheetRow++;
            }
        }
...
//                output.append(formattedValue.replace("\"", "\"\""));
                ...
                line.add(formattedValue.replace("\"", "\"\""));
//                output.append('"');
            }
        }
    }
    ...
    private SXSSFWorkbook outputWorkbook;

    private List<String> line = new ArrayList<>();

    private WenShuParser parser = new WenShuParser();
     ...
     */
    public XLSX2CSV(OPCPackage pkg, PrintStream output, int minColumns, SXSSFWorkbook outputWorkbook) {
    ...
    public void setOPCPackage(OPCPackage pkg) {
        this.xlsxPackage = pkg;
    }
     ...
     * @throws SAXException if parsing the XML data fails.
     */
    public void process(SheetToCSV sheetToCSV) throws IOException, OpenXML4JException, SAXException {
...
//        }
        ...
        XLSX2CSV xlsx2csv = new XLSX2CSV(p, System.out, minColumns, wb);
        XLSX2CSV.SheetToCSV sheetToCSV = xlsx2csv.new SheetToCSV();
        for (int year = 2011; year <= 2020; year++) {
            System.out.println("year = " + year);
            File xlsxFile = new File("D:\\work\\wenshu\\result\\民事劳动争议-" + year + ".xlsx");
//        if (!xlsxFile.exists()) {
...
//        }
            ...
            try {
                ...
                xlsx2csv.setOPCPackage(p);
                xlsx2csv.process(sheetToCSV);
            } finally {
                p.revert();
            }
        }

        FileOutputStream out = new FileOutputStream("D:\\work\\wenshu\\result\\result.xlsx");
        wb.write(out);
        out.close();
